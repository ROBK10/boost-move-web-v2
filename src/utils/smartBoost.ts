...
export function smartBoostV1(capacityScore: number, context: Context) {
  ...
  const title = band === "low" ? "Lav kapasitet i dag" : band === "stable" ? "Stabil kapasitet i dag" : "Høy kapasitet i dag"
  const message = band === "low" ? "Kroppen signaliserer at du bør holde det enkelt." : band === "stable" ? "Du virker stabil. Små ting kan gi litt ekstra." : "Du er i flyt. Hold det jevnt og smart."
