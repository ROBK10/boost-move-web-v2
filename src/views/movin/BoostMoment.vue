  ...
  <div class="p-6 pt-10 min-h-full flex flex-col bg-zinc-50 pb-32 relative overflow-hidden">
    ...
    <div class="flex-1 flex flex-col gap-6 relative z-10">
      ...
      <div class="bg-white rounded-[32px] p-6 shadow-sm border border-zinc-100 flex items-center justify-between">
        <div>
          <div class="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-1">
            Daglige Boost
          </div>
      ...
      <div class="flex-1">
        <transition-group name="fade" tag="div">
          ...
          <div
            v-if="step === 'energy'"
            key="energy"
            class="bg-white rounded-[32px] p-8 shadow-lg shadow-zinc-200/50 border border-zinc-100 flex flex-col justify-between min-h-[400px]"
          >
            <div class="space-y-8">
              ...
              <div>
                <p class="text-lg font-medium text-zinc-600 mb-6 leading-relaxed">
                  Hvordan er energien din akkurat nå?
                </p>
                ...
                <div class="relative h-64 flex items-end justify-center gap-2 px-2 w-full">
                  <button
                    ...
                    :class="[
                      'w-full rounded-t-xl transition-all duration-300 relative group',
                      level <= energy ? 'bg-zinc-900' : 'bg-zinc-100 hover:bg-zinc-200'
                    ]"
                    :style="{ height: (level * 10) + '%' }"
                  >
                    <div
                      v-if="level === energy"
                      class="absolute -top-8 left-1/2 -translate-x-1/2 bg-zinc-900 text-white text-xs font-bold py-1 px-2 rounded-lg whitespace-nowrap"
                    >
                      {{ level }} / 10
                    </div>
                  </button>
            ...
            <button
              @click="step = 'action'"
              class="w-full bg-zinc-900 text-white h-16 rounded-[24px] font-bold text-lg flex items-center justify-center gap-2 hover:bg-zinc-800 transition-colors shadow-xl shadow-zinc-200 mt-8"
            >
          ...
          <div
            ...
            key="action"
            class="bg-zinc-900 rounded-[32px] p-8 shadow-xl shadow-zinc-900/20 border border-zinc-800 flex flex-col justify-between min-h-[400px] text-white relative overflow-hidden"
          >
            ...
            <div class="relative z-10">
              <div class="flex items-center justify-between mb-8">
                ...
                <button
                  @click="refreshAction"
                  class="p-3 bg-white/5 hover:bg-white/10 rounded-full transition-colors text-zinc-400 hover:text-white"
                >
                  🔁
                </button>
              </div>
              ...
              <div class="py-10 text-center">
                <p class="text-sm font-bold text-emerald-400 uppercase tracking-widest mb-4">
                  Din oppgave
                </p>
                <h3 class="text-3xl font-bold leading-tight md:text-4xl">
                  {{ action }}
                </h3>
              </div>
            </div>
            ...
            <button
              ...
              class="w-full bg-white text-zinc-900 h-16 rounded-[24px] font-bold text-lg flex items-center justify-center gap-2 hover:bg-zinc-100 transition-colors shadow-lg relative z-10"
            >
              Ferdig
            </button>
          </div>
          ...
          <div
            v-else
            key="done"
            class="bg-emerald-50 rounded-[32px] p-8 shadow-lg shadow-emerald-100 border border-emerald-100 flex flex-col items-center justify-center text-center min-h-[400px]"
          >
            <div class="w-24 h-24 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center mb-6 text-4xl">
              ✓
            </div>
            ...
            <p class="text-zinc-600 font-medium max-w-[200px] mx-auto">
              Du har fullført dagens boost og investert i din helse.
            </p>
...
const step = ref<'energy' | 'action' | 'done'>('energy')
