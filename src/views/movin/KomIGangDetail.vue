...
import { getKomIGangDay, getNextKomIGangDayId } from "@/data/komIGangContent"
...
function complete() {
  // V1: bare gå tilbake til lista.
  // (senere: lagre fullført status i store/localStorage)
  const nextId = getNextKomIGangDayId(id.value)
  if (nextId) {
    // du kan velge om du vil gå til neste dag automatisk:
    // router.push(`/movin/kom-i-gang/${nextId}`)
    router.push("/movin/kom-i-gang")
    return
  }
  router.push("/movin/kom-i-gang")
...
.complete {
  ...
  border: none;
  border-radius: 18px;
  background: rgba(185, 255, 0, 0.95); /* “complete grønn” vibe */
  color: #111827;
  font-weight: 900;
  cursor: pointer;
}
